...
class PetnalyAnimal(models.Model):
    ...
    color_id = fields.Many2one('petnaly.dog.color', string='Color / Markings')
    life_stage_id = fields.Many2one(
        'petnaly.dog.life.stage', string='Current Life Stage',
    ...
    def _compute_life_stage(self):
        ...
        stages = self.env['petnaly.dog.life.stage'].search([])
        for rec in self:
            stage = self.env['petnaly.dog.life.stage']
