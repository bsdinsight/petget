...
class PetnalyAnimal(models.Model):
    ...
    sire_id = fields.Many2one(
        'petnaly.animal', string='Sire',
        domain="[('sex', '=', 'male'), ('species', '=', species)]",
    )
    dam_id = fields.Many2one(
        'petnaly.animal', string='Dam',
        domain="[('sex', '=', 'female'), ('species', '=', species)]",
    )

    # --- Documents ---
    document_ids = fields.One2many(
        'petnaly.document', 'animal_id', string='Documents',
    )
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get('system_id') or vals['system_id'] == _('New'):
                vals['system_id'] = (
                    self.env['ir.sequence'].next_by_code('petnaly.animal')
                    or _('New')
                )
