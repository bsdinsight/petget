...
class PetgetReservation(models.Model):
    _inherit = 'petget.reservation'

    followup_ids = fields.One2many(
        'petget.followup', 'reservation_id', string='Follow-ups',
    )
    ...
    def _generate_followups(self):
        templates = self.env['petget.followup.template'].search([])
        Followup = self.env['petget.followup']
        for rec in self:
