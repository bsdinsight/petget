...
class PetnalyAnimal(models.Model):
    ...
    litter_id = fields.Many2one('petnaly.litter', string='Birth Litter', index=True)
    ...
    heat_cycle_ids = fields.One2many(
        'petnaly.heat.cycle', 'animal_id', string='Heat Cycles',
    )
    pregnancy_ids = fields.One2many(
        'petnaly.pregnancy', 'dam_id', string='Pregnancies',
    )
