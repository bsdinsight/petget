...
class PetnalyLitter(models.Model):
    _name = 'petnaly.litter'
    ...
    pregnancy_id = fields.Many2one('petnaly.pregnancy', string='Pregnancy', index=True)
    dam_id = fields.Many2one(
        'petnaly.animal', string='Dam', related='pregnancy_id.dam_id',
        store=True, readonly=True,
    )
    sire_id = fields.Many2one(
        'petnaly.animal', string='Sire', related='pregnancy_id.mating_id.sire_id',
        store=True, readonly=True,
    )
    ...
    offspring_ids = fields.One2many(
        'petnaly.animal', 'litter_id', string='Puppies',
    )
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get('name') or vals['name'] == _('New'):
                vals['name'] = (
                    self.env['ir.sequence'].next_by_code('petnaly.litter')
                    or _('New')
                )
