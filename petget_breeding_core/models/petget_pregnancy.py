...
class PetgetPregnancy(models.Model):
    _name = 'petget.pregnancy'
    ...
    dam_id = fields.Many2one(
        'petget.animal', string='Dam', required=True, ondelete='cascade',
        index=True, domain=[('sex', '=', 'female')], tracking=True,
    )
    mating_id = fields.Many2one('petget.mating', string='Mating')
    ...
    litter_id = fields.Many2one(
        'petget.litter', string='Litter', readonly=True, copy=False,
    )
    ...
    def action_create_litter(self):
        ...
        if self.litter_id:
            ...
            litter = self.env['petget.litter'].create({
