...
class PetgetMating(models.Model):
    ...
    sire_id = fields.Many2one(
        'petget.animal', string='Sire', domain=[('sex', '=', 'male')],
        tracking=True,
    )
    ...
    heat_cycle_id = fields.Many2one(
        'petget.heat.cycle', string='Heat Cycle',
    ...
    pregnancy_id = fields.Many2one(
        'petget.pregnancy', string='Pregnancy', readonly=True, copy=False,
    )
    ...
    def action_create_pregnancy(self):
        ...
        if self.pregnancy_id:
            ...
            preg = self.env['petget.pregnancy'].create({
