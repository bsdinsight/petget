...
class PetnalyAnimal(models.Model):
    ...
    dam_litter_ids = fields.One2many(
        'petnaly.litter', 'dam_id', string='Litters as Dam',
    )
    ...
    def _get_compliance_rule(self):
        self.ensure_one()
        state = self.company_id.petnaly_au_state
        if not state:
            return self.env['petnaly.compliance.au.rule'].browse()
        species = self.species if self.species in ('dog', 'cat') else 'dog'
        return self.env['petnaly.compliance.au.rule'].search(
            [('state', '=', state), ('species', '=', species)], limit=1)

    @api.depends('date_of_birth', 'microchip', 'status', 'sex', 'species',
                 'company_id', 'company_id.petnaly_au_state', 'dam_litter_ids')
